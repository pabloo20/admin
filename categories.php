...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        ...
        if ($action == 'insert_product') {
		  ...
		  // INSERT INTO products_attributes (products_id, options_id, options_values_id, options_values_price, price_prefix) VALUES ("35", "1", "1", "0", "");
// Improve Product Attributes/Item Editor begin
/*for ($i = 0, $n = sizeof($attributes); $i < $n; $i++) {
		$values = split("_", $attributes[$i]);
		tep_db_query("INSERT INTO products_attributes (products_id, options_id, options_values_id, options_values_price, price_prefix) VALUES (" . $products_id . ", '" . $values[0] . "', '" . $values[1] . "','". $att_price[$i]. "','". $plus[$i]."')");
	}*/
...
//if ( isset($HTTP_GET_VARS['pID']) && $HTTP_GET_VARS['pID'] != '' ) {
	...
	$attributes = $HTTP_POST_VARS['attributes'];
	...
	tep_db_query("INSERT INTO testowe (test1) VALUES ('" . $attributes[$i]. "')");
	  ...
	  if($values[2] == "" && $values[3] == "") {
	    tep_db_query("INSERT INTO products_attributes (products_id, options_id, options_values_id, options_values_price, price_prefix) VALUES (" . (int)$products_id . ", '" . $values[0] . "', '" . $values[1] . "','". $att_price[$i]. "','". $plus[$i]. "')");
	  } else {
	    $values[2] = "\"" . $values[2] . "\"";
	    $values[3] = "\"" . $values[3] . "\"";
	    tep_db_query("INSERT INTO products_attributes (products_id, options_id, options_values_id, options_values_price, price_prefix) VALUES (" . (int)$products_id . ", '" . $values[0] . "', '" . $values[1] . "','". $att_price[$i]. "','". $plus[$i]. "')");

...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        ...
        if ($action == 'insert_product') {
		  ...
		  tep_db_query("TRUNCATE testowe");
		tep_db_query("INSERT INTO testowe (test1) VALUES ('insert_product')");	
		tep_db_query("INSERT INTO testowe (test1) VALUES ('"  . (int)$products_id . "')");
		
		// Improve Product Attributes/Item Editor begin
	//tep_db_query("DELETE FROM products_attributes WHERE products_id = " . (int)$products_id);
	
		$attributes = $_POST['attributes'];
	
		for ($i = 0, $n = sizeof($attributes); $i < $n; $i++) {
		tep_db_query("INSERT INTO testowe (test1) VALUES ('"  .  $attributes[$i] . "')");
			$values = explode("_", $attributes[$i]);
			$aa = sizeof($values);
			tep_db_query("INSERT INTO testowe (test1, test2) VALUES ('" . $n. "' , '" . $aa ."')");
			tep_db_query("INSERT INTO testowe (test1, test2) VALUES ('" . $values[0]. "' , '" . $values[1] ."')");
			
			if($values[0] !== ""){
				tep_db_query("INSERT INTO products_attributes (products_id, options_id, options_values_id, options_values_price, price_prefix) VALUES (" . (int)$products_id . ", '" . $values[0] . "', '" . $values[1] . "','0','0')");
			}
	  }
...
/** AJAX Attribute Manager  end **/	
			...
			tep_db_query("TRUNCATE testowe");
			tep_db_query("INSERT INTO testowe (test1) VALUES ('update_product')");	
			tep_db_query("INSERT INTO testowe (test1) VALUES ('"  . (int)$products_id . "')");	
			tep_db_query("DELETE FROM products_attributes WHERE products_id = " . (int)$products_id);
...
//if ( isset($_GET['pID']) && $_GET['pID'] != '' ) {
	...
	tep_db_query("INSERT INTO testowe (test1) VALUES ('" . $attributes[$i]. "')");
	  ...
	  if($values[0] !== ""){
	  	tep_db_query("INSERT INTO testowe (test1) VALUES ('" . $values[0]. "')");
			tep_db_query("INSERT INTO testowe (test1) VALUES ('" . $values[1]. "')");
	    tep_db_query("INSERT INTO products_attributes (products_id, options_id, options_values_id, options_values_price, price_prefix) VALUES (" . (int)$products_id . ", '" . $values[0] . "', '" . $values[1] . "','0','0')");
	  ...
	  while($options_values = tep_db_fetch_array($options)) {
        ...
        $suboptions = tep_db_query("SELECT * FROM products_options_values POV, products_options_values_to_products_options POVtoPO WHERE language_id = '" . $languages_id . "' AND POVtoPO.products_options_id = '" . $options_values['products_options_id'] . "' AND POVtoPO.products_options_values_id = POV.products_options_values_id ORDER BY POV.products_options_values_id");
		 ...
		 while($suboptions_values = tep_db_fetch_array($suboptions)) {
		 					$product_id = $_GET['pID'];
					$prodoptions = tep_db_query("SELECT * FROM products_attributes WHERE products_id = '" . $product_id . "' AND options_id = '" . $options_values['products_options_id'] . "' AND options_values_id = '" . $suboptions_values['products_options_values_id'] . "'" );
					...
					echo "\n" . "<input type=\"checkbox\" name=\"attributes[".$i."]\" value=\"" . $options_values['products_options_id'] . "_" . $suboptions_values['products_options_values_id'] . "\"" . $checked . ">" . $suboptions_values['products_options_values_name'] . "<br>";// . 
					
					//" <input size=\"6\" type=\"text\" name=\"att_price[".$i."]\" value=\"".$prodoptions_values['options_values_price']."\"> <select name=\"plus[".$i."]\"><option value=\"\"><option value=\"+\"". $price_checked_plus .">+<option value=\"-\"". $price_checked_minus .">-</select><br>";
        $i++;}
